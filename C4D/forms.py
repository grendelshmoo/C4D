...
class SearchForm(forms.Form):
    ...
    document_date_range = forms.DateField(label="Document Date Range", required=False)
    dd_sel = forms.ChoiceField(widget=RadioSelect(attrs={'onclick': 'ddRange()'}), choices=[['dd_range', 'Range'], ['dd_eq', 'Equals'], ['dd_less', 'Less than'], ['dd_greater', 'Greater than']], label="Document Date Criteria", required=False)
    recording_date = forms.DateField(label="Recording Date", required=False)
    recording_date_range = forms.DateField(label="Recording Date Range", required=False)
    rd_sel = forms.ChoiceField(widget=RadioSelect(attrs={'onclick': 'rdRange()'}), choices=[['rd_range', 'Range'], ['rd_eq', 'Equals'], ['rd_less', 'Less than'], ['rd_greater', 'Greater than']], label="Recording Date Criteria", required=False)
